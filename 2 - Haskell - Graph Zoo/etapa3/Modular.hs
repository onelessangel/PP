...
-}
mapSingle :: (a -> a) -> [a] -> [[a]]
mapSingle f xs = reverse $ create 0 []
    where
        size = length xs
        makeList first elem rest = first ++ [f elem] ++ rest
        create len res
            | len == size = res
            | otherwise = create (len + 1) (newList : res)
            where
                (first, rest) = splitAt len xs
                newList = makeList first (head rest) (tail rest)
...
-}
addElemToPart elem ys = concatMap (addElem elem) ys

addElem elem [] = [[[elem]]]
addElem elem (x : xs) = res
    where
        res = ((elem : x) : xs) : restRes
        restRes = map (x :) (addElem elem xs)
        
partitions :: [a] -> [[[a]]]
partitions [] = [[]]
partitions (x : xs) = addElemToPart x $ partitions xs
...
{-
    *** TODO10 ***
    ...
    False
-}
refIn set graph = inNeighbors (S.elemAt 0 set) graph
refOut set graph = outNeighbors (S.elemAt 0 set) graph
results set graph = S.map (\x -> (inNeighbors x graph == refIn set graph) && (outNeighbors x graph == refOut set graph)) set

isModule :: Ord a
         => S.Set a
         -> Graph a
         -> Bool
isModule set graph = (S.notMember False results)
    where
        refIn = inNeighbors (S.elemAt 0 set) graph
        refOut = outNeighbors (S.elemAt 0 set) graph
        results = S.map (\x -> (inNeighbors x graph == refIn) && (outNeighbors x graph == refOut)) set
