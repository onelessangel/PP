...
search f node graph = algo [node] [node]
    where
        algo list visited
            | null list = visited
            | null goodNeigh = algo (tail list) visited
            | otherwise = algo newList (visited `union` newList)
            where
                v = head list
                neighbors = S.toList $ outNeighbors v graph
                goodNeigh = filter (\x -> not (x `elem` visited)) neighbors
                newList = f list goodNeigh
...
bfs = search f
    where f queue neighbors = (tail queue) ++ neighbors
...
dfs = search f
    where f stack neighbors = (head neighbors) : stack
...
-}
-- Numara cate elemente inermediare se afla in lista data.
countElems :: Eq a => a -> [a] -> Int
countElems to list
    | not $ to `elem` list = 0
    | otherwise = length $ fst $ span (/= to) $ tail list
...
countIntermediate from to graph
    | lenBfs == 0 = Nothing
    | otherwise = Just (lenBfs, lenDfs) 
    where
        bfsRes = bfs from graph
        dfsRes = dfs from graph
        lenBfs = countElems to bfsRes
        lenDfs = countElems to dfsRes
