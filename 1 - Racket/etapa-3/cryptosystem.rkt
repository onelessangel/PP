...
(define (9-tuple e f)
  (list (a1 e f) (b1 e f) (c1 e f)
        (a2 e f) (b2 e f) (c2 e f)
        (a3 e f) (b3 e f) (c3 e f)))

(define (key n)
  (let* ([tuple (get-nth-quadruple n)] [e (list-ref tuple e-index)] [f (list-ref tuple f-index)])
    (map (λ (x) (modulo x 27)) (9-tuple e f))))
...
(define (message->codes message)
  (map (λ (x) (max 0 (- (char->integer x) 96))) (string->list message)))
...
(define (extend-key key size)
  (let ([original-k key])
    (let extend ([key key] [size size])
      (if (< size (length key))
          (take key size)
          (extend (append key original-k) size)))))
...
(define (transformation func format final-format)
  (λ (code key)
    (let ([code (format code)])
      (final-format (map func code (extend-key key (length code)))))))

(define encrypt-codes
  (transformation encrypt-func no-changes no-changes))
(define decrypt-codes
  (transformation decrypt-func no-changes no-changes))
...
(define encrypt-message
  (transformation encrypt-func (curry message->codes) (curry codes->message)))
(define decrypt-message
  (transformation decrypt-func (curry message->codes) (curry codes->message)))
