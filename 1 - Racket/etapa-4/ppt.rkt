...
; transforma o lista intr-un stream
(define (list->stream L)
  (if (empty? L)
      empty-stream
      (stream-cons (car L) (list->stream (cdr L)))))

; creeaza un stream cu 3 tupluri - copiii tuplului dat:
; 1. converteste lista de transformari intr-un stream
; 2. aplica stream-map pentru a lua fiecare transformare si a o aplica pe tuplul dat,
;    rezultand 3 tupluri copii
(define (tuple-children-stream tuple)
  (stream-map (λ (f) (f tuple)) (list->stream trans-T)))

; 1. adaug in coada tuplul (3 4 5)
; 2. retin primul element din coada in q-head si restul cozii in q-tail
; 3. adaug q-head la flux si adaug in coada copiii tuplului q-head
(define ppt-stream-in-tree-order
  (let loop ([queue (list '(3 4 5))])
    (let ([q-head (stream-first queue)] [q-tail (stream-rest queue)])
      (stream-cons q-head (loop (stream-append q-tail (tuple-children-stream q-head)))))))
...
(define (pairs G H)
  (let loop-h ([H H] [index-h 1])    ; h >= 1
    (let loop-g ([G G] [index-g 1])  ; 1 <= g <= h
      (if (<= index-g index-h)
          (stream-cons (cons (stream-first G) (stream-first H)) (loop-g (stream-rest G) (add1 index-g)))
          (loop-h (stream-rest H) (add1 index-h))))))
...
; fluxul numerelor impare
(define odd-stream (stream-filter odd? naturals))

(define gh-pairs-stream
  (stream-filter (λ (pair) (equal? (gcd (car pair) (cdr pair)) 1))
                 (pairs odd-stream (stream-rest odd-stream))))
...
; calculeaza patratul unui numar
(define (sqr x)
  (* x x))
...
; creeaza tuplul (a b c) pentru o pereche (g, h)
(define create-tuple
  (λ (pair)
...
(define ppt-stream-in-pair-order
  (stream-map create-tuple gh-pairs-stream))